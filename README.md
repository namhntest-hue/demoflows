...
- **Thu gọn**: dòng `#voucherTrigger` = icon vé + "Áp dụng ưu đãi" + chữ tóm tắt bên phải ("Chọn hoặc nhập mã" / "Đã chọn N ưu đãi") + chevron. Mobile: 1 dòng trần trong card viền sẵn. Desktop: dòng **có viền** cao 48 trong cột tóm tắt 427px (ở đó không có card con nào khác nên cần viền để đọc ra "bấm được"). Mã đang áp hiện thành **chip có nút ×** ngay dưới dòng đó, gỡ được mà không cần mở panel.
...
- **i18n**: nhãn tĩnh vào từ điển; mọi chuỗi mang số (`Giảm 1,500,000đ`, `Đơn từ …`, `Mua thêm … để dùng mã`, `Đã chọn N ưu đãi`, `· HSD dd/mm/yyyy`) đi bằng **luật regex** 2 chiều. Mã voucher + tên chương trình mang `data-i18n-skip` (wordmark). Sheet nằm ngoài `.screen` nên `open()` gọi `localizeNew(panel)`.
