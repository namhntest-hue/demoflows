...
- **Nhãn: LUÔN là "Ưu đãi & khuyến mãi ( N )"** (chốt 17/08/2026 — tên mục không đổi theo trạng thái), N = số mã **đang đủ điều kiện** (`voucherAvailCount`; mã ẩn và mã chưa đạt mức không tính — đếm cả thứ bấm vào không dùng được thì con số nói dối). Con số chính là lý do để bấm vào: nói "có gì đó cho bạn ở đây" mà không phải mở panel. Số khả dụng đổi theo giỏ nên `refreshCartSummary` **luôn** gọi `renderVoucherUI`, không chỉ khi có mã rớt.
- **Chữ phụ bên phải chỉ báo "Đã áp dụng ( N )"**, mặc định **để trống** (chốt 17/08/2026 — chưa có gì để nói thì đừng nói; bản trước có nudge "Chọn hoặc nhập mã" đã bỏ). Cũng **không in số tiền giảm** ở dòng này.
...
- **i18n**: nhãn tĩnh vào từ điển; mọi chuỗi mang số (`Ưu đãi & khuyến mãi ( N )` ↔ `Offers & promotions ( N )`, `Đã áp dụng ( N )` ↔ `Applied ( N )`, `Giảm 1,500,000đ`, `Đơn từ …`, `Mua thêm … để dùng mã`, `Đã chọn N ưu đãi`, `· HSD dd/mm/yyyy`) đi bằng **luật regex** 2 chiều. Mã voucher + tên chương trình mang `data-i18n-skip` (wordmark). Sheet nằm ngoài `.screen` nên `open()` gọi `localizeNew(panel)`.

## Cookie consent — nội dung theo bản khách duyệt (17/08/2026, CHỈ MOBILE)

Khách gửi nguyên văn 2 block, `index.html` chép đúng chữ (`desktop.html` CHƯA port — xem mục "Vấn đề tồn đọng").
...
- **Cookie consent — nội dung bản khách duyệt (17/08/2026)**: mới áp cho `index.html`. Desktop còn nguyên chữ cũ (`Đồng ý tất cả`, `Tùy chọn cookies`, `Cookies …`, link "Chính sách Cookie") — cùng cấu trúc 2 view nên port là chép chữ + đổi 4 tên nhóm + thêm 3 gạch đầu dòng + link `[data-cg-privacy]`, lưu ý bản desktop đặt cookie bar **bên trái** và có `I18N` riêng.
